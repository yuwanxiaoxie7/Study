...
def validate_zip(path,names):
    ...
    return {'images':len(expected),'errors':[],'sha256':sha(path),'format':'8-bit grayscale PNG, 1024x1024, IDs 1..8'}
...
def submit(model,c,run,checkpoint):
    ...
    with zipfile.ZipFile(Path(c['data_root'])/c['test_zip']) as source,zipfile.ZipFile(tmp,'w',zipfile.ZIP_DEFLATED) as dest:
        ...
        if len(names)!=c['test_count'] or len(set(basenames))!=c['test_count']: raise ValueError(f"Expected {c['test_count']} test images")
