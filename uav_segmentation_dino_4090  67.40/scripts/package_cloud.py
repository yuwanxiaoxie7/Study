...
for folder in ['aic','uavseg','scripts','tests','vendor','configs','artifacts','reference']:
    for p in (root/folder).rglob('*'):
        if p.is_file() and '__pycache__' not in p.parts and (p.suffix in ['.py','.json','.md','.txt'] or p.name=='LICENSE'):
            ...
            selected.append(p)
selected+=list(root.glob('*.md'))+list(root.glob('*.sh'))+list(root.glob('*.bat'))+[root/'pipeline.py',root/'predict_test2.py',root/'requirements.txt',root/'submission/validation.json']
...
print('Round 2 prediction: bash run_test2.sh --test-zip /path/to/test_2.zip --checkpoint /path/to/best.pt')
