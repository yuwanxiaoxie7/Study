...
from .common import ROOT,read,sha,save,NORMALIZATION
...
def tensor(im,c):
    ...
    return original_tensor(im,NORMALIZATION[c['weight_source']])
def split():
...
def audit(c,output):
    ...
    with zipfile.ZipFile(root/c['test_zip']) as z:
        ...
        if len(entries)!=500 or len({Path(n).name for n in entries})!=500: raise ValueError('Test1 requires 500 unique PNGs')
        for n in entries:
...
class Crops(torch.utils.data.Dataset):
    ...
    def __init__(self,c,epoch,order):
        ...
        old['normalization']=NORMALIZATION[c['weight_source']]
