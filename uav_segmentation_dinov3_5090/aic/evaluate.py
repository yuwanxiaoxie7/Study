...
def predict(model,image,c):
    ...
    for y in positions(h,tile,c['stride']):
        for x in positions(w,tile,c['stride']):
            batch=tensor(image.crop((x,y,x+tile,y+tile)),c).unsqueeze(0).to(device)
            with autocast(c,device.type):
                p=model(batch)[0].float()
                if c.get('tta','none')=='hflip': p=(p+model(batch.flip(-1))[0].float().flip(-1))/2
    ...
    with zipfile.ZipFile(path) as z:
        ...
        for name in z.namelist():
            ...
            with Image.open(io.BytesIO(raw)) as im:
                a=np.asarray(im)
                if im.mode!='L' or im.size!=(1024,1024) or a.ndim!=2 or a.min()<1 or a.max()>8: raise ValueError('Invalid mask')
    return {'images':len(expected),'errors':[],'sha256':sha(path),'bytes':Path(path).stat().st_size,'format':'8-bit grayscale PNG, 1024x1024, IDs 1..8'}
...
def submit(model,c,run,checkpoint):
    ...
    (out/'submission.sha256').write_text(report['sha256']+'  submission.zip\n',encoding='ascii')
    print(f'提交文件（检查通过）: {final.resolve()}',flush=True)
    print(f'ZIP bytes: {report["bytes"]}; SHA256: {report["sha256"]}',flush=True)
    return final
