...
import argparse,contextlib,datetime,json,os,platform,subprocess,sys,time,traceback,urllib.request
...
def pretrained(c):
    ...
    if not target.exists():
        ...
        with urllib.request.urlopen(url,timeout=120) as src,tmp.open('wb') as dst:
            total=int(src.headers.get('Content-Length',0)); downloaded=0; last=0.; started=time.monotonic()
            while True:
                chunk=src.read(8*1024*1024)
                if not chunk: break
                dst.write(chunk)
                downloaded+=len(chunk); now=time.monotonic()
                if now-last>=5 or downloaded==total:
                    percent=f'{downloaded/total:.1%}' if total else 'total unknown'
                    print(f'Weights: {downloaded/2**20:.0f} MiB ({percent}), {downloaded/2**20/max(now-started,.001):.1f} MiB/s',flush=True)
                    last=now
            if total and downloaded!=total: raise RuntimeError('Incomplete weight download; rerun to retry')
...
def main():
    ...
    if platform.system()!='Linux': raise RuntimeError('5090工程请上传Linux云端，运行 bash run_all.sh。')
